...
KEYRING_ACCOUNT = "google-api-key"
...
class SettingsDialog(QDialog):
    def __init__(
        ...
        self.api_input.setPlaceholderText("Google Gemini API key")
        link = QLabel(
            '<a style="color:#60a5fa; text-decoration:none" '
            'href="https://aistudio.google.com/apikey">Google AI Studio API key ↗</a>'
        )
...
class TranslatorWindow(QWidget):
    ...
    def _load_api_key(self) -> str:
        ...
        if not getattr(sys, "frozen", False):
            ...
            candidate = str(
                dotenv.get("GOOGLE_API_KEY") or dotenv.get("GEMINI_API_KEY") or ""
            ).strip()
            if candidate:
                try:
    ...
    def _connection_timed_out(self) -> None:
        if not self.process or self.connected:
            return
        self._set_status("ULANISH TIMEOUT — QAYTA BOSING", "#ef4444")
        self.last_engine_error = (
            "Gemini 12 soniya ichida javob bermadi. Internet va API key’ni tekshiring."
        )
        self.process.kill()
