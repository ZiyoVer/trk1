...
Gemini Live connection through the product backend so revoked clients cannot
reach the paid service even after patching the executable.

## Release controls already implemented

- Gemini and license values are stored in Keychain/Credential Manager.
...
5. Move the Gemini credential and Live websocket relay server-side before
