...
- first-run Gemini API key setup stored in Keychain/Credential Manager;
...
3. The user is prompted for their own Gemini key on first launch.
...
14. `Ikkalasi` mode runs incoming and outgoing Gemini Live sessions in parallel,
...
- Client-side checks raise the cost of casual copying but cannot provide hard
  revocation against a patched binary; paid production requires a server-side
  Gemini Live relay and server-owned credentials.
