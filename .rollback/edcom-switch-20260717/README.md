...
Zoom yoki Google Meet ovozini tanlangan input qurilmadan olib, Gemini Live
Translate yordamida real vaqtda tanlangan til va audio outputga uzatadi.
Audio va transcript diskka yozilmaydi.
Default tarjima ovozi: `Charon`.
Gemini’dan keladigan 24 kHz mono tarjima ovozi output qurilmaning native sample
...
bilan birga o‘rnatadi. Birinchi ochilishda foydalanuvchi o‘z Gemini API key’ini
...
- `BOSHLASH` — Gemini translatorni ulaydi;
...
- `Avtomatik` manba — bitta meetingda inglizcha va ruscha navbatma-navbat
  gapirilsa, Gemini audiodagi tilni o‘zi aniqlab tanlangan target tilga o‘giradi;
...
Boshqa Gemini ovozini sinash uchun:
...
- Mode: `Ikkalasi`
- Eshitish: `BlackHole 2ch → Gemini → fizik speaker`
- Gapirish: `fizik mikrofon → Gemini → BlackHole 16ch`
...
- Meeting audio Google Gemini API’ga yuboriladi; ishtirokchilarga xabar berish kerak.
- Model preview bo‘lgani uchun API nomi va limitlari keyinchalik o‘zgarishi mumkin.
- Lokal installer’ni reverse engineeringdan 100% yopib bo‘lmaydi. Release secret
  scan, short-lived license va signing hooklari bor; pullik productionda Gemini
  Live websocket/proxy server tomonga ko‘chirilishi kerak. Batafsil: `SECURITY.md`.
